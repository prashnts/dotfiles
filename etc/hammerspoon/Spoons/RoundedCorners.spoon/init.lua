...
obj.level = hs.canvas.windowLevels["cursor"]
...
function obj:init()
    ...
    self.spaceWatcher = hs.spaces.watcher.new(function() self:screensChanged() end)
end
...
function obj:deleteAllCorners()
    hs.fnutils.each(self.corners, function(corner) corner:delete(0.4) end)
    self.corners = {}
end
...
function obj:render()
    ...
    hs.fnutils.each(screens, function(screen)
        ...
        for _,data in pairs(cornerData) do
            self.corners[#self.corners+1] = hs.canvas.new({x=data.frame.x, y=data.frame.y, w=radius, h=radius})
                :appendElements(
                    ...
                    { type="resetClip", })
                :behavior(hs.canvas.windowBehaviors.stationary)
                :level(self.level)
                :show(0.4)
        end
    end)
